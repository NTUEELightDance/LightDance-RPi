...
CmdParser::printHelps() const
{
   ...
   // CmdMap::iterator iter;
    ...
    for(auto iter = _cmdMap.begin(); iter != _cmdMap.end(); iter ++)//wwwwwwww
        iter->second->help();
    return ;
}
...
CmdParser::parseCmd(string& option)
{
   ...
   string str = _history.back();//enter and add to history first?  yes
   // TODO...
    ...
    for (size_t i = 0; i < str.size() ; i++)
    {
        ...
        if (e != 0)
        {
            ...
            if (end != str.size())  //there is ' '
            {
                if (myStrNCmp(possible_cmd + e->getOptCmd(), str.substr(0, end), possible_cmd.size() ) == 0 )
            ...
            {
                if (myStrNCmp(possible_cmd + e->getOptCmd(), str, possible_cmd.size() ) == 0)
                {
                    option = "";
                    return e;
                }
            }
            //when to compare the _optCmd ? here or exec ?
            //if(mtStrNCmp( substr(str, i - str.size() + 1), e->_optCmd,  ))
            //break;
        }
    }
    cerr << "Illegal command!! (" << possible_cmd << ")" << endl;
   return NULL;
}
...
// Please note:
// ------------
// 1. The mandatory part of the command string (stored in _cmdMap) must match   wwwwww
// 2. The optional part can be partially omitted.                               wwwwww
// 3. All string comparison are "case-insensitive".              myStrNCmp?     wwwwww
//
CmdExec*                    //www
